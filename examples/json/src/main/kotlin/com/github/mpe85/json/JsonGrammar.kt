...
open class JsonGrammar : AbstractGrammar<JsonElement>() {
    ...
    open fun jsonString(): Rule<JsonElement> =
        sequence(
            doubleQuote(),
            zeroOrMore(sequence(testNot(doubleQuote()), anyChar())),
            push { JsonString(checkNotNull(it.previousMatch?.toString())) },
            doubleQuote(),
            whitespaces(),
        )

    open fun jsonNumber(): Rule<JsonElement> =
        sequence(
            sequence(
                sequence(
                    optional(unaryMinus()),
                    oneOrMore(digit()),
                    optional(sequence(char('.'), oneOrMore(digit()))),
                ),
    ...
    open fun jsonNull(): Rule<JsonElement> = sequence(string("null"), push(JsonNull), whitespaces())

    open fun jsonBoolean(): Rule<JsonElement> =
        sequence(
            strings("true", "false"),
    ...
    open fun jsonElement(): Rule<JsonElement> =
        choice(jsonString(), jsonNumber(), jsonNull(), jsonBoolean(), jsonObject(), jsonArray())

    open fun jsonObject(): Rule<JsonElement> =
        sequence(
            curlyBracketLeft(),
            sequence(
                jsonString(),
            ...
            zeroOrMore(
                sequence(
                    ...
                    jsonElement(),
                    command {
                        (peek(2, it) as JsonObject).properties[(pop(1, it) as JsonString).value] =
                            pop(it)
                    },
                )
            ),
    ...
    open fun jsonArray(): Rule<JsonElement> =
        sequence(
            squareBracketLeft(),
            jsonElement(),
