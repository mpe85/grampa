...
open class CalculatorGrammar : AbstractGrammar<Int>() {
    ...
    open fun expression(): Rule<Int> =
        term() + zeroOrMore(add() + term() + push { pop(it) + pop(it) })

    open fun term(): Rule<Int> =
        factor() + zeroOrMore(mul() + factor() + push { pop(it) * pop(it) })
    ...
    open fun integer(): Rule<Int> =
        optional(char('-')) +
            oneOrMore(digit()) +
            push { it.previousMatch.toString().toInt() } +
            whitespaces()
