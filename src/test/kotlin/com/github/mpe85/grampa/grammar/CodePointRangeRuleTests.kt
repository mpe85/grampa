    ...
    StringSpec({
        ...
        "CodePointRange rule throws exception when lower bound is higher than higher bound" {
            checkAll(Arb.set(legalCodePoints(), 2..2)) { codePoints ->
                val (low, high) =
                    codePoints.toList().map { it.value }.sorted().let { it[0] to it[1] }
