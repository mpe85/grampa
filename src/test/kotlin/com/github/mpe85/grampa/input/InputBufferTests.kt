    ...
    StringSpec({
        ...
        "Get character" {
            buffers.forAll { buffer ->
                ...
                shouldThrow<IndexOutOfBoundsException> { buffer.getChar(-1) }
                shouldThrow<IndexOutOfBoundsException> { buffer.getChar(6) }
            }
        }
        "Get code point" {
            buffers.forAll { buffer ->
                ...
                shouldThrow<IndexOutOfBoundsException> { buffer.getCodePoint(-1) }
                shouldThrow<IndexOutOfBoundsException> { buffer.getCodePoint(6) }
            }
        }
        "Get length" { buffers.forAll { buffer -> buffer.length shouldBe 6 } }
        "Sub sequence" {
            buffers.forAll { buffer ->
                ...
                shouldThrow<IndexOutOfBoundsException> { buffer.subSequence(-2, 1) }
                shouldThrow<IndexOutOfBoundsException> { buffer.subSequence(0, 7) }
                shouldThrow<IndexOutOfBoundsException> { buffer.subSequence(5, 3) }
            }
        }
        "Get position" {
            buffers.forAll { buffer ->
