...
class ActionRuleTests : StringSpec({
    ...
    "Action function is correctly converted to action rule" {
        ...
        rule.toString() shouldBe "ActionRule(action=(com.mpe85.grampa.context.RuleContext<kotlin.Unit>) -> kotlin.Boolean, skippable=false)"
    }
    "Action is correctly converted to action rule" {
        val rule = Action<Unit> { true }.toRule()
        rule.toString() shouldBe "ActionRule(action=fun com.mpe85.grampa.rule.Action<T>.run(com.mpe85.grampa.context.RuleContext<T>): kotlin.Boolean, skippable=false)"
    }
