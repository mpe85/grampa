                ...
                Parser(grammar).apply {
                    codePoints.forEach { cp ->
                        run(String.format("%c", cp.value)).apply {
                            ...
                            restOfInput shouldBe String.format("%c", cp.value)
                        }
                    }
                }
            }
    ...
    "NoneOfCodePoints rule matches codepoint not in collection" {
        checkAll(Arb.set(legalCodePoints(), 2..10)) { codePoints ->
            grammars(codePoints.map { it.value }.drop(1)).forEach { grammar ->
                Parser(grammar).run(String.format("%c", codePoints.first().value)).apply {
                    matched shouldBe true
                    matchedEntireInput shouldBe true
                    matchedInput shouldBe String.format("%c", codePoints.first().value)
